...
export const recordFileResource = async (fileInfo) => {
  try {
    const [fileResource] = await db.insert(fileResourcesTable).values({
 ...
 */
export const markFileAsUsed = async (fileUrl) => {
  try {
    // 更新文件状态
    await db.update(fileResourcesTable)
      .set({
        status: 'active',
        updatedAt: new Date()
      })
    ...
    console.log(chalk.green(`[文件标记为已使用] ${fileUrl}`));
    return true;
  } catch (error) {
    console.error('[标记文件使用失败]', error);
    return false;
  }
};
 ...
 * @returns {Promise<boolean>} 是否成功
 */
export const markFileAsUnused = async (fileUrl) => {
  try {
    // 更新文件状态为未使用
    await db.update(fileResourcesTable)
      .set({
    ...
    console.log(chalk.yellow(`[文件标记为未使用] ${fileUrl}`));
    return true;
  } catch (error) {
    console.error('[标记文件未使用失败]', error);
    return false;
  }
};
...
export const cleanupSingleFile = async (fileResource) => {
  try {
    ...
    if (deleteSuccess) {
      ...
      console.warn(chalk.yellow(`[文件删除失败] ${fileResource.fileName} - ${fileResource.fileUrl}`));
      return false;
    }
  } catch (error) {
...
export const batchCleanupFiles = async (batchSize = 50) => {
  try {
    ...
    console.log(chalk.green(`[文件清理完成] 总计: ${result.total}, 成功: ${result.success}, 失败: ${result.failed}`));
    return result;
  } catch (error) {
