...
function init($, _, locale, Handlebars, apiProject, apiData, Prism, sampleRequest, semver) {
  ...
  function sortFields(fields_object) {
    $.each(fields_object, function (key, fields) {
      ...
      var objects = fields.filter(function (item) {
      ...
      for (var object of objects) {
        ...
        var objectFields = fields.filter(function (item) {
          return item.field.indexOf(object.field + '.') > -1;
        });
      ...
      var nonObjects = fields.filter(function (item) {
        return item.field.indexOf('.') === -1 && item.type !== 'Object';
      });
